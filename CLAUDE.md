...
- **diff.rs** — Wraps git2. `DiffState::load()` discovers the repo, computes HEAD-to-workdir diff (staged + unstaged) for the given file, and returns parsed hunks and per-line marks. Returns `Err` if not in a git repo or no diff exists — callers use `.ok()` to degrade gracefully.
